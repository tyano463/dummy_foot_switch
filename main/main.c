...
// D1 is GPIO3
#define GPIO_INPUT_PIN_SEL (1ULL << 3)

#define NOTIFY_COMMAND (0x30)

/// Declare the static function
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param);
static void pedal_check(void *param);
static void generate_notify_data(void);

#define GATTS_SERVICE_UUID_TEST_A "2ae0ffc2-dd80-4bab-a3a2-01c6f6193761"
#define GATTS_CHAR_UUID_TEST_A "2ae0ffc2-dd81-4bab-a3a2-01c6f6193761"
...
static bool parse_uuid_128(const char *uuid, uint8_t *store)
{
    bool ret = false;
    int len = strlen(uuid);
    uint8_t upper, lower;
    ...
    for (int i = 0; i < 36; i++)
    {
        ...
        upper = uuid[i];
        lower = uuid[i + 1];
        *(--p) = ((c2b(upper) & 0xf) << 4) | ((c2b(lower) & 0xf) << 0);
        i++;
    }
    ret = (p == store);
error_return:
...
{
    bool ret;
    ret = parse_uuid_128(GATTS_SERVICE_UUID_TEST_A, &adv_service_uuid128[0]);
    d("a: %d", ret);
}
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
        ...
        ret = parse_uuid_128(GATTS_SERVICE_UUID_TEST_A, gl_profile_tab[PROFILE_A_APP_ID].service_id.id.uuid.uuid.uuid128);
        d("a: %d", ret);
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        parse_uuid_128(GATTS_CHAR_UUID_TEST_A, gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid128);
...
void app_main(void)
{
    ...
    xTaskCreate(pedal_check, "Pedal Check", 4 * 1024, NULL, 5, NULL);
    return;
}

static void generate_notify_data(void)
{
    notify_data[0] = NOTIFY_COMMAND;
    notify_data[1] = (pedal_value & 0xff);
}
...
static void pedal_check(void *param)
{
    ...
    c.pin_bit_mask = GPIO_INPUT_PIN_SEL;
    c.pull_down_en = 0;
    c.pull_up_en = 1;
    gpio_config(&c);

    pedal_value = !gpio_get_level(GPIO_NUM_3);
    int cur;
    ...
    while (true)
    {
        ...
        cur = !gpio_get_level(GPIO_NUM_3);
        if (cur == pedal_value)
            goto next;

        pedal_value = cur;
        generate_notify_data();
        struct gatts_profile_inst *p = &gl_profile_tab[PROFILE_A_APP_ID];

        esp_ble_gatts_send_indicate(p->gatts_if, p->conn_id, p->char_handle,
                                    sizeof(notify_data), notify_data, false);
